...
add_action( 'save_post', 'mip_insert_postdata' );
...
function mip_add_custom_box() {
	...
	if(empty($post_name)) {
		...
		$post_types = explode(',', get_option( 'mip_post_type' ));
		foreach($post_types as $p_type) {
			add_meta_box('mip_sidebar', 'Images For this Post', 'mip_render_images', $p_type, 'side', 'default');			
		}
	}
}
...
?>
  ...
  <form method="post" action="options.php">
    ...
    <table class="form-table">
      ...
      <td>
				<input style="width: 360px; height: 36px;" type="text" name="mip_post_type" value="<?php echo get_option( 'mip_post_type' ); ?>"/>
				<div style="color: red;">* For multiple post types use comma seperated value, i.e.  post_type1, post_type_2</div>
			</td>
      </tr>
    </table>
    <?php submit_button(); ?>
